...
export default class AppStore extends Store {
	...
	queryPage = async ({ pageNum, pageSize} = {pageNum: 0, pageSize: 10}) => {
		...
		if (pageRes.code === Const.SUCCESS_CODE) {
			this.transaction(() => {
				...
				// 设置分页数据
				this.dispatch('info:setPageData', pageRes.context.${classNameFirstLower}VOPage);
				// 设置当前页码
	...
	onEdit = async (id) => {
		const editData = this.state()
			.get('dataList')
			.find((v) => v.get('${table.idColumn.columnNameLower}') == id);
		this.transaction(() => {
	...
	onSave = async () => {
		...
		let result;
		if (formData.get('${table.idColumn.columnNameLower}')) {
	...
	onExportModalShow = (exportModalData) => {
		this.dispatch('info:exportModalShow', fromJS({
			...exportModalData,
			visible: true,
	...
	onExportByIds = () => {
		...
		return this._onExport({ ${table.idColumn.columnNameLower}List: checkedIds });
	};
		...
		return new Promise((resolve) => {
			setTimeout(() => {
				...
				if (token) {
					...
					const exportHref = Const.HOST + `/${classNameLowerCase}/export/<@jspEl 'encrypted'/>`;
					// 新窗口下载
